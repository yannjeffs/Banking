...
import {
  Users, CreditCard, Landmark, ArrowRightLeft,
  TrendingUp, Clock, Wallet, BarChart3, RefreshCw,
...
export default function AdminDashboard() {
  ...
  const rejectLoan = async (loanId: number) => {
    await api.put(`/loan/${loanId}/reject`, { reason: 'Rejeté depuis le dashboard' });
    setLoans(prev => prev.filter(l => l.loanId !== loanId));
    refresh();
  };
  ...
  const txChartData = [
    { name: 'Virements',  total: txs.filter(t => t.transactionType === 'Virement').length  },
    { name: 'Dépôts',     total: txs.filter(t => t.transactionType === 'Depot').length     },
    { name: 'Retraits',   total: txs.filter(t => t.transactionType === 'Retrait').length   },
  ];
    ...
    <div className="flex min-h-screen bg-gray-50">
      ...
      <main className="ml-60 flex-1 p-8">
        ...
        <div className="flex items-center justify-between mb-8">
          <div>
            <h2 className="text-2xl font-bold text-slate-800">Dashboard Administrateur</h2>
            <p className="text-sm text-gray-500 mt-0.5">Vue globale de la banque</p>
          </div>
          <button
            onClick={refresh}
            disabled={refreshing}
        ...
        <div className="grid grid-cols-2 xl:grid-cols-4 gap-4 mb-8">
          <KpiCard
            label="Clients" value={stats?.totalUsers ?? 0}
            icon={<Users className="w-6 h-6" />}
            bg="bg-blue-100" iconCol="text-blue-600"
            sub="Comptes clients actifs"
          />
          <KpiCard
            label="Comptes bancaires" value={stats?.totalAccounts ?? 0}
            icon={<CreditCard className="w-6 h-6" />}
            bg="bg-emerald-100" iconCol="text-emerald-600"
            sub={`${fmt(stats?.totalDeposits ?? 0)} XAF total`}
          />
          <KpiCard
            label="Prêts actifs" value={stats?.activeLoans ?? 0}
            icon={<Landmark className="w-6 h-6" />}
            bg="bg-orange-100" iconCol="text-orange-600"
            sub={`${fmt(stats?.totalLoanAmount ?? 0)} XAF en cours`}
          />
          <KpiCard
            label="Transactions" value={stats?.totalTransactions ?? 0}
            icon={<ArrowRightLeft className="w-6 h-6" />}
            bg="bg-purple-100" iconCol="text-purple-600"
        ...
        {(stats?.pendingLoans ?? 0) > 0 && (
          <div className="flex items-center gap-3 bg-amber-50 border border-amber-200 rounded-2xl px-5 py-4 mb-6">
            ...
            <p className="text-sm text-amber-700 font-medium">
              {stats?.pendingLoans} demande{(stats?.pendingLoans ?? 0) > 1 ? 's' : ''} de prêt en attente d'approbation
            </p>
          </div>
        )}
        ...
        <div className="grid grid-cols-1 xl:grid-cols-3 gap-6 mb-6">
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
            ...
            <div className="flex flex-col gap-3">
              {[
                { label: 'En attente', value: stats?.pendingLoans ?? 0,                                     color: 'bg-yellow-500' },
                { label: 'Actifs',     value: stats?.activeLoans  ?? 0,                                     color: 'bg-green-500'  },
                { label: 'Total',      value: stats?.totalLoans   ?? 0,                                     color: 'bg-blue-500'   },
                { label: 'Encours',    value: `${fmt(stats?.totalLoanAmount ?? 0)} XAF`, color: 'bg-orange-500' },
              ].map(item => (
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
            ...
            <div className="divide-y divide-gray-50">
              {loans.length === 0 ? (
                ...
                loans.map(loan => (
                  <div key={loan.loanId} className="flex items-center gap-3 px-5 py-3.5 hover:bg-gray-50">
                    <div className="flex-1 min-w-0">
                      <div className="text-sm font-semibold text-slate-700 truncate">
                        {loan.ownerName}
                      </div>
                      <div className="text-xs text-gray-400 mt-0.5">
                        {fmt(loan.amount)} XAF · {loan.termMonths} mois · {loan.interestRate}%
                      </div>
                    </div>
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
            ...
            <div className="divide-y divide-gray-50">
              {txs.length === 0 ? (
                ...
                txs.map(tx => (
                  <div key={tx.transactionId} className="flex items-center gap-3 px-5 py-3 hover:bg-gray-50">
                    <div className={`w-8 h-8 rounded-full flex items-center justify-center shrink-0 text-xs font-bold
                      ${tx.transactionType === 'Virement' ? 'bg-blue-100 text-blue-600'
                      : tx.transactionType === 'Depot'    ? 'bg-green-100 text-green-600'

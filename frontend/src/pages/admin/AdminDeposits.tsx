...
export default function AdminDeposits() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const endpoint =
        mode === "deposit" ? "/admin/deposit" : "/admin/withdraw";
      const res = await api.post<{ message: string; newBalance: number }>(
        endpoint,
        { ...form, amount: Number(form.amount) },
      );
      setSuccess(
        `${res.data.message} — Nouveau solde : ${res.data.newBalance.toLocaleString("fr-FR")} XAF`,
      );
      setSelected((prev) =>
        ...
        <div className="flex items-center gap-3 mb-8">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500">
              Opérations manuelles sur les comptes
            </p>
          </div>
        </div>
